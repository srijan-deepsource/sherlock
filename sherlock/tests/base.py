...
class SherlockBaseTest(unittest.TestCase):
    def setUp(self):
        ...
        try:
            with open(excluded_sites_path, "r",
                      encoding="utf-8") as excluded_sites_file:
        ...
        for username in username_list:
            ...
            for site, result in results.items():
                with self.subTest(f"Checking Username '{username}' "
                    ...
                    if (self.skip_error_sites
                            == True) and (result["status"].status
                                          == QueryStatus.UNKNOWN):
    ...
    def coverage_total_check(self):
        ...
        for site, site_data in self.site_data_all.items():
            if (site_data.get("username_claimed") is
                    None) or (site_data.get("username_unclaimed") is None):
